...
my_string trim_leading_zeros(my_string num) {
   ...
   //Checking zero my_string
   for (my_string::size_type i = 0; i <= (num.size()-1); ++i){
      if ((num.at(i)) != '0') {
            zero_num = false;
            break;
         }
   ...
   //Main function trimming zero
     for ( my_string::size_type i = 0; i <= (num.size()); ++i) {
         if (num.at(i) == '0'){
              num.replace(i,1,"*");
         }
...
my_string add(my_string lhs, my_string rhs) {
   //Copy lhs and rhs
    my_string copy_lhs = lhs;
    my_string copy_rhs = rhs;

   lhs = trim_leading_zeros(lhs);
   rhs = trim_leading_zeros(rhs);
   my_string result_add ="";
   int carry = 0;
   int sum;
   ...
   //Regarrange length of my_string, adding 2 postive number
   if ((lhs.at(0) != '-') && (rhs.at(0) !='-')) {
   if (lhs.length() != rhs.length()) {
      if (lhs.length() > rhs.length()) {
         while (lhs.length() != rhs.length()){
            rhs.insert(0,"0" );  //add '0' in the my_string
         }
      }
      else if (lhs.length() < rhs.length())
         while (lhs.length() != rhs.length()){
            lhs.insert(0,"0" );  //add '0' in the my_string
            }
    }
      ...
      for (int i = (lhs.length()-1) ; i >= 0 ; i--) {
         ...
         carry = 0;
         if(sum > 9) {
            result_add.insert(0, my_string (1, decimal_to_digit (sum % 10)));
            carry = 1;
         }
         else if (sum <= 9) {
            result_add.insert(0, my_string (1, decimal_to_digit (sum)));
         }
         sum = 0;
...
my_string subtract(my_string lhs, my_string rhs) {
   ...
   for (int i = (lhs.length()-1) ; i >= 0 ; i--) { //CHECKING AGAIN
         ...
         result_sub.insert(0,my_string(1,decimal_to_digit(num_sub)));
         num_sub = 0;
      }
...
my_string multiply(my_string lhs, my_string rhs) {
   ...
   my_string temp_result = "";
   my_string multi_result = "0";
   int carry = 0;
   int result = 0;
   int number = 0;
   ...
   for ( int i = rhs.length()-1; i >= 0; --i){
         for ( int j = lhs.length()-1; j >= 0; --j){
            ...
            carry = 0;
         if (result > 9) {
            temp_result.insert(0,my_string(1,decimal_to_digit(result % 10)));
            carry =  result / 10;
         }
         else {
         temp_result.insert(0,my_string(1,decimal_to_digit(result)));
         }
         result = 0;
         }
         if (carry != 0) {
            temp_result.insert(0,my_string(1,decimal_to_digit(carry)));
            carry = 0;
         }
